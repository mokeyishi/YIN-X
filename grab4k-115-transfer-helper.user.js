...
(function () {
  ...
  const AutoAction = {
    ...
    closeCurrentTabSoon(reasonText) {
      if (this.closingScheduled) return;
      this.closingScheduled = true;

      Toast.show(reasonText || '转存成功，1.5 秒后自动关闭页面', 'success', CONFIG.closeDelayMs);
      setTimeout(() => {
        window.close();
        setTimeout(() => {
          if (!document.hidden) {
  ...
  async function transferRow(row, triggerBtn) {
    ...
    if (result?.ok) {
      ...
      if (!result.skip) {
        AutoAction.closeCurrentTabSoon('转存成功，1.5 秒后自动关闭页面');
      }
  ...
  function maybeAutoTransferSingle(rows) {
    if (AutoAction.autoClicked || rows.length !== 1) return;
    const btn = rows[0].querySelector('.g4k-transfer-btn');
    if (!btn) return;

    AutoAction.autoClicked = true;
    Toast.show('检测到仅 1 个资源，1.5 秒后自动转存', 'info', CONFIG.autoClickDelayMs);
    setTimeout(() => {
      btn.click();
