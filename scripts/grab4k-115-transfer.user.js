...
// @version      5.5.0
// @description  在 Grab4K 内容页为 115 资源提供一键/批量转存与列表选择转存，支持自动定位到资源区与成功后自动关页
...
(function () {
  ...
  const CONFIG = {
    ...
    autoClickDoneFlagPrefix: 'g4k_auto_clicked_single_row',
  };
  ...
  function scheduleAutoClose() {
    ...
    Toast.show(`转存成功，${(CONFIG.autoCloseDelayMs / 1000).toFixed(1)} 秒后自动关闭标签页`, 'success', CONFIG.autoCloseDelayMs);
    setTimeout(closeCurrentTab, CONFIG.autoCloseDelayMs);
  }
  ...
  async function transferRow(row, triggerBtn) {
    ...
    if (result?.ok) {
      Toast.show(result.msg, result.skip ? 'skip' : 'success', 2500);
      if (!result.skip) scheduleAutoClose();
    } else if (result) {
      Toast.show(result.msg, 'error', 2800);
    }
  ...
  function maybeAutoClickSingleResource(rows) {
    if (rows.length !== 1) return;
    const pageClickFlag = `${CONFIG.autoClickDoneFlagPrefix}:${location.pathname}${location.search}`;
    if (sessionStorage.getItem(pageClickFlag) === '1') return;

    const btn = rows[0].querySelector('.g4k-transfer-btn');
    if (!btn) return;

    sessionStorage.setItem(pageClickFlag, '1');
    setTimeout(() => {
      Toast.show('检测到单资源，自动发起一键转存', 'info', 1800);
      btn.dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true, view: window }));
    }, CONFIG.autoClickDelayMs);
  }
  ...
  function injectTransferButtons() {
    ...
    maybeAutoClickSingleResource(rows);
  }
