...
// @version      5.5.0
// @description  在 Grab4K 内容页为 115 资源提供一键/批量转存与列表选择转存，支持自动定位到资源区，支持成功后自动关闭标签页
...
(function () {
  ...
  async function transferRow(row, triggerBtn, options = {}) {
    const { autoCloseOnSuccess = false } = options;
    ...
    if (result?.ok) {
      ...
      if (autoCloseOnSuccess && !result.skip) {
        AutoActions.scheduleCloseTab();
      }
    } else if (result) {
  ...
  function showBatchSelectionModal(rows) {
    ...
    modal.querySelector('#g4k-start-transfer').onclick = async () => {
      ...
      for (let i = 0; i < selectedIndexes.length; i += 1) {
        ...
        const result = await transferRow(row, row.querySelector('.g4k-transfer-btn'), { autoCloseOnSuccess: false });
  ...
  function maybeAutoClickSingleResource(rows) {
    if (AutoActions.singleAutoClickDone || rows.length !== 1) return;
    const btn = rows[0]?.querySelector('.g4k-transfer-btn');
    if (!btn) return;

    AutoActions.singleAutoClickDone = true;
    setTimeout(() => {
  ...
  function injectTransferButtons() {
    ...
    rows.forEach(row => {
      ...
      btn.onclick = async event => {
        ...
        await transferRow(row, btn, { autoCloseOnSuccess: true });
      };
