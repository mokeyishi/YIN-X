...
(function () {
  ...
  function scheduleCloseCurrentTab() {
  ...
  async function transferRow(row, triggerBtn, options = {}) {
    const { autoCloseOnSuccess = true } = options;
    ...
    if (result?.ok) {
      ...
      if (autoCloseOnSuccess && !result.skip) {
        Toast.show('1.5 秒后自动关闭当前标签页', 'info', CONFIG.autoCloseDelay);
        scheduleCloseCurrentTab();
      }
    } else if (result) {
  ...
  function showBatchSelectionModal(rows) {
    ...
    modal.querySelector('#g4k-start-transfer').onclick = async () => {
      ...
      if (ok > 0 && fail === 0) {
        Toast.show('批量转存成功，1.5 秒后自动关闭当前标签页', 'info', CONFIG.autoCloseDelay);
        scheduleCloseCurrentTab();
      }
    };
  }
