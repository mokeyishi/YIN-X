...
(function () {
  ...
  function closeCurrentTabSoon() {
    Toast.show('转存成功，1.5 秒后自动关闭标签页', 'success', CONFIG.autoCloseDelay);
    setTimeout(() => {
      window.close();
      setTimeout(() => {
        window.open('', '_self');
  ...
  async function transferRow(row, triggerBtn, options = {}) {
    const { shouldCloseOnSuccess = false } = options;
    ...
    if (shouldCloseOnSuccess && result?.ok && !result.skip) {
      closeCurrentTabSoon();
    }
  ...
  function showBatchSelectionModal(rows) {
    ...
    modal.querySelector('#g4k-start-transfer').onclick = async () => {
      ...
      for (let i = 0; i < selectedIndexes.length; i += 1) {
        ...
        const result = await transferRow(row, row.querySelector('.g4k-transfer-btn'), { shouldCloseOnSuccess: false });
  ...
  function maybeAutoClickSingleTransfer(rows) {
    if (STATE.autoClickDone) return;
    if (rows.length !== 1) return;
    const btn = rows[0].querySelector('.g4k-transfer-btn');
    if (!btn) return;

    STATE.autoClickDone = true;
    Toast.show('检测到单个资源，1.5 秒后自动转存', 'info', CONFIG.autoClickDelay);
    setTimeout(() => {
      btn.click();
  ...
  function injectTransferButtons() {
    ...
    rows.forEach(row => {
      ...
      btn.onclick = async event => {
        ...
        await transferRow(row, btn, { shouldCloseOnSuccess: true });
      };
    ...
    maybeAutoClickSingleTransfer(rows);
  }
